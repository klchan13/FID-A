function [spect] = sim_mega_mixedRF(spin_sys, more_params)
...
refTp1=6.91;%duration of refocusing pulses[ms]
refTp2=6.91;%duration of refocusing pulses[ms]
editTp=14;
...
taus=[TE1/2 - refTp1/2,...
      TE2/4 - refTp1/2- editTp/2,...
      TE2/4  + TE1/2 - refTp2/2 - editTp/2,...
      TE2/4 - refTp2/2 - editTp/2,...
...
elseif strcmp(pulse_type, 'shaped')
    pulse_seq = @sim_mega_shaped_mixedRF;
end

% ************END OF INPUT PARAMETERS**********************************

%Load RF waveforms
switch refocWaveform1
    case 'fmref07_fida.pta'
        refRF1=io_loadRFwaveform_klc(refocWaveform1,'ref',0);
        refRF1.tbw = 38.280;
        refRF1.tw1 = 9.932;
...
switch refocWaveform2
    case 'fmref07_fida.pta'
        refRF2=io_loadRFwaveform_klc(refocWaveform2,'ref',0);
        refRF2.tbw = 38.280;
        refRF2.tw1 = 9.932;
    case 'gtst1203_sp.pta'
...
Gx=(refRF1.tbw/(refTp1/1000))/(gamma*thkX/10000); %[G/cm]
Gy=(refRF2.tbw/(refTp2/1000))/(gamma*thkY/10000); %[G/cm]
            ...
            for EP2=1:length(editPhCyc2)
                for RP1=1:length(refPhCyc1)
                    for RP2=1:length(refPhCyc2)
                        ...
                        outON_posxy_epc_rpc{X}{Y}{EP1}{EP2}{RP1}{RP2}=pulse_seq(Npts,sw,Bfield,lw,taus,sys,edit_angle,...
                            editRFon,editTp,editPhCyc1(EP1),editPhCyc2(EP2),...
                            refRF1,refRF2,refTp1,refTp2,Gx,Gy,x(X),y(Y),refPhCyc1(RP1),refPhCyc2(RP2));
                        outOFF_posxy_epc_rpc{X}{Y}{EP1}{EP2}{RP1}{RP2}=pulse_seq(Npts,sw,Bfield,lw,taus,sys,edit_angle,...
