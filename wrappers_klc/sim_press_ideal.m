...
% 
% where, in [X Y], X is the phase of the first refocus`ing pulse and Y is
...
function out = sim_press_ideal(n,sw,Bfield,linewidth,sys,tau1,tau2,rf,tp,x,y,Gx,Gy,phCyc1,phCyc2)
...
d = sim_rotate(d,H,180,phCyc1);
%d=sim_shapedRF(d,H,RF,tp,180,90+phCyc1,Gx,dx);          %1st shaped 180 degree refocusing pulse
%d=sim_shapedRFwithH(d,H,RF,tp,180,90+phCyc1,Gx,dx);          %1st shaped 180 degree refocusing pulse
d=sim_evolve(d,H,(tau1+tau2)/2000);                     %Evolve by (tau1+tau2)/2
d = sim_rotate(d,H,180,phCyc2);
%d=sim_shapedRF(d,H,RF,tp,180,90+phCyc2,Gy,dy);          %2nd shaped 180 degree refocusing pulse
%d=sim_shapedRFwithH(d,H,RF,tp,180,90+phCyc2,Gy,dy);          %2nd shaped 180 degree refocusing pulse
